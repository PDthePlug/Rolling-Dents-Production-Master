...
import { InquiryForm } from "@/components/rolling-dents/inquiry-form";
import { ADDRESS, EMAIL, FACEBOOK_URL, INSTAGRAM_URL, MAPS_URL, pageMetadata, PHONE_DISPLAY, whatsappUrl } from "@/lib/rolling-dents";

export const metadata = pageMetadata("Contact Rolling Dents | Laser Park Roodepoort", "Call, WhatsApp, email, submit an enquiry or get directions to Rolling Dents at 1510 Zeiss Road, Laser Park, Roodepoort.", "/contact", "/media/workshop-entrance.webp");

export default function ContactPage() {
  return (
    <main>
      <section className="contact-hero">
        <div><p className="eyebrow"><span />CONTACT ROLLING DENTS</p><h1>Bring the damage.<br /><em>Leave with the finish.</em></h1><p>Start a vehicle assessment, ask the workshop a question, or use the direct contact channels at 1510 Zeiss Road in Laser Park.</p><div className="hero-actions"><Button asChild className="orange-button hero-button"><Link href="/estimate">Start assessment <ArrowRight /></Link></Button><Button asChild variant="outline" className="dark-outline hero-button"><a href={whatsappUrl()} target="_blank" rel="noreferrer"><MessageCircle /> WhatsApp us</a></Button></div></div>
      ...
      <section id="existing-repair" className="service-decision">
        <div><p className="section-kicker orange-kicker">VEHICLE ALREADY WITH ROLLING DENTS?</p><h2>Ask the workshop for the current update.</h2><p>Until a repair is connected to the customer tracking service, WhatsApp and the workshop line remain the source for the most current repair update. Include your name, registration and any assessment, claim or job reference you have.</p></div>
        <div className="service-decision-card"><MessageCircle /><h3>Existing repair update</h3><p>Use the enquiry form below to create a reference, or speak to the workshop directly if the update is urgent.</p><Button asChild className="orange-button content-button"><a href={whatsappUrl("Hi Rolling Dents, my vehicle is already with the workshop. I would like an update on the repair.")} target="_blank" rel="noreferrer">WhatsApp the workshop <ArrowRight /></a></Button></div>
      </section>

      <section className="inquirySection section-light">
        <div><p className="section-kicker">NOT A NEW DAMAGE ASSESSMENT?</p><h2>Send a general customer enquiry.</h2><p>Existing repair updates, insurance questions, restoration projects and other workshop questions can be recorded separately from a new vehicle assessment.</p><Link className="text-link dark-text-link" href="/estimate">New vehicle damage? Start an assessment <ArrowRight /></Link></div>
        <InquiryForm />
      </section>
